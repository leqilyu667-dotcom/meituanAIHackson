        ...
        <div v-for="msg in filteredMessages" :key="msg.id" class="card mb-3 cursor-pointer" @click="openChat(msg)">
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="mt-0.5 truncate text-sm text-cocoa">{{ msg.content }}</p>
              <p class="mt-0.5 text-[10px] text-cocoa/60">商家 · 美甲师 · 您</p>
            </div>
            <button @click.stop="goToSalon(msg)" class="shrink-0 rounded-full bg-primary-50 px-3 py-1.5 text-[11px] font-medium text-primary-600 active:scale-95 transition">
              进店
            </button>
          </div>
        </div>
...
const goToSalon = (msg) => {
  if (msg.salonId) {
    router.push(`/salon-detail/${msg.salonId}`)
  }
}
